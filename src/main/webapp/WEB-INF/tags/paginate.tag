...
<div class="pagination">
  ...
  <ul>
    ...
    <c:if test="${currentPage > 1}">
      ...
      <li><a href="${url}" title="Previous Page">${preLabel}</a></li>
    </c:if>
    ...
    <c:if test="${currentPage - 2 > 1}">
      ...
      <li><a href="${url}" >${currentPage - 2}</a></li>
    </c:if>
    <c:if test="${currentPage - 1 > 1}">
      <c:url var="url" value="${RTLUri}" >
          <c:param name="${pageVarName}" value="${currentPage - 1}" />
      </c:url>
      <li><a href="${url}">${currentPage - 1}</a></li>
    </c:if>
    
    <c:url var="url" value="${RTLUri}" >
        <c:param name="${pageVarName}" value="${currentPage}" />
    </c:url>
    <li><a href="${url}" class="active">${currentPage}</a></li>    
    ...
    <c:if test="${currentPage  != pages}">
      ...
      <li><a href="${url}" title="Next Page">${nextLabel}</a></li>
    </c:if>
    <c:url var="url" value="${RTLUri}" >
        <c:param name="${pageVarName}" value="${pages}" />
    </c:url>
